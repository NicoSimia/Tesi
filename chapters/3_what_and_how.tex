...
Nella collaborazione quotidiana con il secondo stagista, abbiamo suddiviso il lavoro per scena piuttosto che per tipo di attività: ciascuno di noi seguiva l'implementazione della navigazione da \textit{controller} su un sottoinsieme di scene, per evitare di lavorare sugli stessi file e ridurre il rischio di conflitti in fase di \textit{merge}. Prima di ogni \textit{commit} su una scena condivisa, ci scambiavamo una revisione rapida del codice, più per allineare stile e struttura delle modifiche che come controllo formale di qualità.
...
La collaborazione tra i due \textit{branch} si basava su una regola semplice: comunicare preventivamente su quali scene o file ciascuno stesse lavorando, per poi dividersi le \textit{task} di conseguenza ed evitare di modificare gli stessi file in parallelo. Prima di integrare il proprio lavoro nel \textit{branch} principale creato a inizio \textit{stage}, verificavamo che le modifiche non introducessero errori nel codice.
