...
Il secondo passaggio, da Unity 2021 a Unity 6, ha richiesto interventi più sostanziali. Alcune dipendenze sono state aggiornate automaticamente dall'\textit{engine}, senza richiedere intervento diretto: \textit{DOTween}, una libreria molto diffusa nell'ecosistema Unity per la gestione semplificata della fisica 2D, rientra in questo caso, poiché il progetto la includeva come file \texttt{.dll} (una libreria già compilata) anziché come pacchetto Unity, e l'\textit{engine} la riconosceva quindi automaticamente attraverso il \textit{Package Manager}.
